...
class Create extends Component {
  ...
  handleInputChange = event => {
    ...
    if (name.indexOf("keyword") >= 0) {
      ...
      if (theKeys.indexOf(value) < 0) {
        // console.log(name);
        let inputKey = parseFloat(name.replace("keyword", ""));
        // console.log(inputKey);
        theKeys[inputKey -1] = value;
      }
      this.setState({
        keywords: theKeys
      });
  ...
  handleFormSubmit = event => {
    ...
    if (this.state.huntName && this.state.location && this.state.userId && (this.state.keywords).length >= 3) {
      API.saveHunt({
          ...
          keywords: this.state.keywords.filter(Boolean)
      })
      .then(huntData => {
          ...
          <Col lg="6" md="8" sm="12">
            ...
            <Form onSubmit={this.handleFormSubmit} action="/api/hunts">
              ...
              <FormGroup>
                ...
                <Input onChange={this.handleInputChange} type="text" name="keyword1" placeholder="Keyword 1" />
              </FormGroup>
              <FormGroup>
                <Input onChange={this.handleInputChange} type="text" name="keyword2" placeholder="Keyword 2" />
              </FormGroup>
              <FormGroup>
                <Input onChange={this.handleInputChange} type="text" name="keyword3" placeholder="Keyword 3" />
              </FormGroup>
              <FormGroup>
                <Input onChange={this.handleInputChange} type="text" name="keyword4" placeholder="Keyword 4" />
              </FormGroup>
              <FormGroup>
                <Input onChange={this.handleInputChange} type="text" name="keyword5" placeholder="Keyword 5" />
              </FormGroup>
